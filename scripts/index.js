...
$(document).ready(function () {
	...
	$('input').keyup(function () {
		...
		if (
			regexEmail.test(inputEmailLogin.value) &&
			inputPasswordLogin.value.trim().length > 0
		) {
	...
	$('input').keyup(function () {
		...
		if (
			...
			inputLastName1.value.trim().length > 1 &&
			inputLastName2.value.trim().length > 1 &&
	...
	$('input').blur(function () {
		...
		if ($(this).hasClass('name')) {
			if ($(this).val().length === 0) {
				...
				$(this)
					.siblings('span.error')
					.text('Debe tener almenos 2 caracteres')
					.fadeIn()
					.parent('.form-group')
					.addClass('hasError');
				usernameError = true;
		...
		if ($(this).hasClass('password')) {
			if ($(this).val().length < 8) {
				$(this)
					.siblings('span.error')
					.text('Debe tener almenos 8 caracteres')
					.fadeIn()
					.parent('.form-group')
					.addClass('hasError');
				passwordError = true;
	...
	$('form.signup-form').submit(function (event) {
		...
		if (
			...
			$(
				'.name, .email, .lastName1, .lastName2, .email, .password'
			).blur();
		} else {
